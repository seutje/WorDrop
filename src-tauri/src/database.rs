...
pub(crate) fn migrate(connection: &mut Connection) -> Result<(), String> {
    ...
    if version < 4 {
        let transaction = connection.transaction().map_err(db_error)?;
        transaction
            .execute_batch(IMAGE_FRAMING_MIGRATION)
            .map_err(db_error)?;
        transaction
    ...
    if version < 6 {
        let transaction = connection.transaction().map_err(db_error)?;
        transaction
            .execute_batch(CLOTHING_FAVORITE_MIGRATION)
            .map_err(db_error)?;
        transaction
...
pub fn set_favorite(
    ...
    let changed = connection
        .execute(
            "UPDATE clothing_items SET favorite=?2, updated_at=strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id=?1",
            params![id, favorite],
        )
        .map_err(db_error)?;
    if changed == 0 {
...
pub fn image_reference_count(connection: &Connection, reference: &str) -> Result<i64, String> {
    connection
        .query_row(
            "SELECT COUNT(*) FROM clothing_items WHERE image_path = ?1 OR display_image_path = ?1",
            [reference],
            |row| row.get(0),
        )
        .map_err(db_error)
...
fn validate(input: &ClothingItemInput) -> Result<(), String> {
    ...
    if !input.crop_zoom.is_finite()
        || input.crop_zoom < 1.0
        || input.crop_zoom > 4.0
        || !input.crop_x.is_finite()
        || !input.crop_y.is_finite()
        || input.crop_x.abs() > 1.0
        || input.crop_y.abs() > 1.0
    {
        return Err("Choose valid image framing values.".into());
    }
    Ok(())
}

fn default_crop_zoom() -> f64 {
    1.0
}
...
mod tests {
    ...
    fn clothing_crud_and_multi_values_work() {
        ...
        assert!(get(&db, "item-1").unwrap().unwrap().favorite);
